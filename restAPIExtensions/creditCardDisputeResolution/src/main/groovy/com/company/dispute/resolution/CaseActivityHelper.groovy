...
trait CaseActivityHelper {
    ...
    def canExecute(String state) {
        return state != BPMNameConstants.NOT_AVAILABLE_STATE &&
    ...
    def valueOfState(String state) {
        switch (state) {
            case BPMNameConstants.REQUIRED_STATE: return 1
            case BPMNameConstants.OPTIONAL_STATE: return 2
            case BPMNameConstants.DISCRETIONARY_STATE: return 3
            case BPMNameConstants.NOT_AVAILABLE_STATE: return 4
    ...
    def getACMStateValue(HumanTaskInstance task, ProcessAPI processAPI) {
        if(task instanceof UserTaskInstance) {
            try {
                def instance = processAPI.getActivityTransientDataInstance(BPMNameConstants.ACTIVITY_STATE_DATA_NAME, task.id)
		...
		BPMNameConstants.OPTIONAL_STATE
    }
}