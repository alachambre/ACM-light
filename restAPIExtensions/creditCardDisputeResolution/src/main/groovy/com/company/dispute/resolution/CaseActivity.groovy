...
class CaseActivity implements RestApiController, CaseActivityHelper, BPMNameConstants {

    private static final String APPLICATION_TOKEN = 'dispute'
    ...
    RestApiResponse doHandle(HttpServletRequest request, RestApiResponseBuilder responseBuilder, RestAPIContext context) {
        ...
        def userPendingTask = processAPI.getPendingHumanTaskInstances(context.apiSession.userId, 0, Integer.MAX_VALUE, ActivityInstanceCriterion.EXPECTED_END_DATE_ASC)
                ...
                .collect{ toActivity(caseId.toLong(), it, getACMStateValue(it,processAPI), pDef, request.contextPath) }
    ...
    def toActivity(long caseId, HumanTaskInstance task, String acmState, ProcessDefinition pDef, String contextPath) {
        [
            name:task.displayName ?: task.name,
            url: canExecute(acmState) ? forge(pDef.name, pDef.version, caseId, task, contextPath) : null,
            description:task.description ?: '',
    ...
    def String forge(String processName, String processVersion, long caseId, ActivityInstance instance, contextPath) {
        if(instance instanceof UserTaskInstance) {
            "$contextPath/portal/resource/taskInstance/$processName/$processVersion/$instance.name/content/?id=$instance.id&displayConfirmation=false&app=$APPLICATION_TOKEN&caseId=$caseId"
        } else if(instance instanceof ManualTaskInstance) {
            "$contextPath/apps/dispute/manualTask?id=$instance.id&app=$APPLICATION_TOKEN&caseId=$caseId"
        }
    }
